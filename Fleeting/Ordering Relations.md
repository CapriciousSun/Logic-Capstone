...
## + Partial Order +
A partially ordered set, also called a *poset*, is a pair $(P, \unlhd)$ where $P$ is some set and $\unlhd$ is an ordering on $P$. A set is partially order if it is reflexive, anti-symmetric, and transitive. 
### ++ Meet and Join ++
...
It can be observed that the direction of the edges indicates the relation from $x$ to $y$. A poset is reflexive, which is shown by the edges pointing in onto a vertex. This diagram could be simplified into a *Hasse diagram*, where the arrows indicating direction all points up and divisors of higher level dividends are connected by other vertices. 
![Hasse diagram | 700](Hasse_Diagram.png)
